...
基于大语言模型（LLM）的上海交通大学水源社区（shuiyuan.sjtu.edu.cn）智能助手。支持网页版聊天、命令行和微信机器人三种使用方式，可自动登录水源论坛与教学信息服务网，实现论坛检索、帖子深度阅读、课程表/成绩查询、外部链接阅读、图片理解等功能。
...
- **图片理解**：主模型不支持视觉输入时，自动调用独立视觉模型（如 qwen-vl 系列）将图片转为文字描述，限制单帖最多 10 张
- **长期记忆**：跨会话记忆用户偏好与关键事实
